...
@property (strong, nonatomic) NSMutableSet *reusableItemViews;
...
- (instancetype)initWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    ...
    if (self) {
        ...
        _backgroundStyle = KSPhotoBrowserBackgroundStyleBlur;
...
- (void)viewDidLoad {
    ...
    self.backgroundEffectView = [[UIVisualEffectView alloc] initWithEffect:[UIBlurEffect effectWithStyle:UIBlurEffectStyleDark]];
    self.backgroundEffectView.frame = self.view.bounds;
    [self.backgroundView addSubview:self.backgroundEffectView];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    CGRect sourceRect = [item.sourceView.superview convertRect:item.sourceView.frame toCoordinateSpace:item.sourceView.window.screen.coordinateSpace];
    
    photoView.imageView.frame = sourceRect;
    
    if ([item.sourceView isKindOfClass:[UIImageView class]]) {
        photoView.imageView.contentMode = ((UIImageView *)item.sourceView).contentMode;
        photoView.imageView.layer.cornerRadius = item.sourceView.layer.cornerRadius;
    }
    ...
    if (_bounces) {
        [UIView animateWithDuration:kSpringAnimationDuration delay:0 usingSpringWithDamping:0.6 initialSpringVelocity:0 options:kNilOptions animations:^{
            ...
            photoView.imageView.layer.cornerRadius = 0;
